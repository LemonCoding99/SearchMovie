...
class HotKeywordServiceTest {
    ...
    void v1TopSynthesis_Success() {
        ...
        given(hotKeywordRepository.findTopKeywords()).willReturn(mockResponse);

        // when
        List<HotKeywordResponse> result = hotKeywordService.v1topSynthesis();

        // then
        assertThat(result).hasSize(1);
        assertThat(result.get(0).getKeyword()).isEqualTo("부장들");
        verify(hotKeywordRepository, times(1)).findTopKeywords();
    }
    ...
    void v2topSynthesis_Success() {
        ...
        // when
        List<HotKeywordResponse> result = hotKeywordService.v2topSynthesis();
    ...
    void v3topSynthesis_CacheHit() {
        ...
        // when
        List<HotKeywordResponse> result = hotKeywordService.v3topSynthesis();

        // then
        assertThat(result.get(0).getKeyword()).isEqualTo("캐시데이터");
        verify(hotKeywordRepository, never()).findTopKeywords(); // DB 조회 안함 확인
    ...
    void v3topSynthesis_CacheMiss() {
        ...
        given(hotKeywordRepository.findTopKeywords()).willReturn(dbData);
        ...
        verify(hotKeywordRepository, times(1)).findTopKeywords(); // DB 조회 발생
    ...
    void v3topPeriod_CacheMiss() {
        ...
        given(hotKeywordRepository.findTopPeriod(any(LocalDateTime.class), any(LocalDateTime.class)))
                .willReturn(List.of());

        // when
        hotKeywordService.v3topPeriod(request);

        // then
        verify(hotKeywordRepository, times(1)).findTopPeriod(any(), any());
